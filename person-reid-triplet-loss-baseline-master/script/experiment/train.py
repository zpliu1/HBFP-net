...
class Config(object):
  def __init__(self):
    ...
    parser.add_argument('--ids_per_batch', type=int, default=16)   #32  16
...
class ExtractFeature(object):
  ...
  def __call__(self, ims):
    ...
    feanorm = torch.norm(fea, p=2, dim=1, keepdim=True)
    fea = fea.div(feanorm.expand_as(fea))
...
def main():
  ...
  criterion_instance = InstanceLoss(gamma=32)
  ...
  for ep in range(start_ep, 470):    #cfg.total_epochs   550  600
    ...
    while not epoch_done:
      ...
      c = b.view(64,1)   #128 64

      y_onehot = torch.FloatTensor(64, 1502)
      # In your for loop
      y_onehot.zero_()
      ...
      '''
      featnorm = torch.norm(feat, p=2, dim=1, keepdim=True)
      feat = feat.div(featnorm.expand_as(feat))
      cir_loss14 = criterion_circle(*convert_label_to_similarity(feat, labels_t)) / 64

      feat1norm = torch.norm(feat1, p=2, dim=1, keepdim=True)
      feat1 = feat1.div(feat1norm.expand_as(feat1))
      cir_loss7 = criterion_circle(*convert_label_to_similarity(feat1, labels_t)) / 64

      feat2norm = torch.norm(feat2, p=2, dim=1, keepdim=True)
      feat2 = feat2.div(feat2norm.expand_as(feat2))
      cir_loss9 = criterion_circle(*convert_label_to_similarity(feat2, labels_t)) / 64

      feat3norm = torch.norm(feat3, p=2, dim=1, keepdim=True)
      feat3 = feat3.div(feat3norm.expand_as(feat3))
      cir_loss11 = criterion_circle(*convert_label_to_similarity(feat3, labels_t)) / 64

      feat4norm = torch.norm(feat4, p=2, dim=1, keepdim=True)
      feat4 = feat4.div(feat4norm.expand_as(feat4))
      cir_loss4 = criterion_circle(*convert_label_to_similarity(feat4, labels_t)) / 64
      '''

      ## instance loss ##
      #loss += criterion_instance(ff, labels) / 64

      featnorm = torch.norm(feat, p=2, dim=1, keepdim=True)
      feat = feat.div(featnorm.expand_as(feat))
      ins_loss14 = criterion_instance(feat, labels_t) / 64

      feat1norm = torch.norm(feat1, p=2, dim=1, keepdim=True)
      feat1 = feat1.div(feat1norm.expand_as(feat1))
      ins_loss7 = criterion_instance(feat1, labels_t) / 64

      feat2norm = torch.norm(feat2, p=2, dim=1, keepdim=True)
      feat2 = feat2.div(feat2norm.expand_as(feat2))
      ins_loss9 = criterion_instance(feat2, labels_t) / 64

      feat3norm = torch.norm(feat3, p=2, dim=1, keepdim=True)
      feat3 = feat3.div(feat3norm.expand_as(feat3))
      ins_loss11 = criterion_instance(feat3, labels_t) / 64

      feat4norm = torch.norm(feat4, p=2, dim=1, keepdim=True)
      feat4 = feat4.div(feat4norm.expand_as(feat4))
      ins_loss4 = criterion_instance(feat4, labels_t) / 64

      
      #loss = 0.5*loss9 + 0.5*loss10 + loss7 + loss8 + loss13 + loss14 + loss11 + loss12 + loss3 + loss4
      loss = 0.5 * ins_loss9 + 0.5 * loss10 + ins_loss7 + loss8 + loss13 + ins_loss14 + ins_loss11 + loss12 + loss3 + ins_loss4
      #loss = loss2
