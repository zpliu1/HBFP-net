...
class Model(nn.Module):
  def __init__(self, last_conv_stride=2):
    ...
    #                            nn.BatchNorm2d(2048),
    #                            nn.ReLU())
    self.conv51 = nn.Sequential(nn.Conv2d(512, 2048, 1, bias=False),
                                nn.BatchNorm2d(2048),
                                nn.ReLU())
    ...
    self.conv52 = nn.Sequential(nn.Conv2d(512, 2048, 1, bias=False),
                                nn.BatchNorm2d(2048),
                                nn.ReLU())
    ...
    self.conv61 = nn.Sequential(nn.Conv2d(512, 2048, 1, bias=False),
                                nn.BatchNorm2d(2048),
                                nn.ReLU())

    self.conv62 = nn.Sequential(nn.Conv2d(512, 2048, 1, bias=False),
                                nn.BatchNorm2d(2048),
                                nn.ReLU())
  ...
  def forward(self, x,label):
    ...
    mid_zeros_1 = torch.zeros(out.size(0), 2048, 192)
    mid_zeros_1 = mid_zeros_1.cuda()

    mid_ones_1 = torch.ones(out.size(0), 2048, 192)
    ...
    '''
    #
    out_1 = out.view(out.size(0),2048,192)
    ...
    #
    out_2 = out1.view(out1.size(0), 2048, 192)
    ...
    #
    out_1_2 = out_2.view(out_2.size(0), 2048, 192)
    ...
    mid_erase1_2 = torch.where(out1_inter_2 > p_yu1_2, mid_ones_0, mid_zeros_0)
    ...
    mid_bb = torch.where(out1_inter_2 > p_bb, mid_ones_0, mid_zeros_0)
    ...
    #
    out_2_2 = out1_2.view(out1_2.size(0), 2048, 192)
    ...
    mid_erase2_2 = torch.where(out2_inter_2 > p_yu2_2, mid_ones_0, mid_zeros_0)
    ...
    mid_nn = torch.where(out2_inter_2 > p_nn, mid_ones_0, mid_zeros_0)
    ...
    #
    '''
    h_oo = h_oo.unsqueeze(dim=2)
    h_oo = h_oo.repeat(1, 1, 192) - 1e-10

    h_oo = torch.where(h1_inter_2 > h_oo, mid_ones_1, mid_zeros_1)

    oo2 = torch.sum(h_oo, 2)
    # print (num1)
    # num1 = 1 / num1

    # num1 = num1.unsqueeze(dim=1).unsqueeze(dim=1)

    h1_oo = torch.mul(h_1, h_oo)
    h1_oo = h1_oo.sum(dim=2)
    # y1_1_m1 = torch.mul(y1_1_m1, num1)
    h1_oo = h1_oo / oo2
    '''
    #
    ...
    #h1_1_2 = torch.sign(h1_1_2) * torch.sqrt(torch.abs(h1_1_2) + 1e-10)
